...
class FlyMH(MetropolisHastings):
    ...
    def __init__(self, dataset, likelihood_function, sample_fraction):
        ...
        self.log_lkhdMAP = likelihood_function.comp_func(
            self.thetaMAP, np.mean(dataset)
        )
        self.meanGradMAP = likelihood_function.comp_gradient(
            self.thetaMAP, np.mean(dataset)
        )
        self.meanHessMAP = likelihood_function.comp_hessian(
            self.thetaMAP, np.mean(dataset)
        )
        self.num_sampled_z = int(np.ceil(self.N * self.sample_fraction))
    ...
    def run(self, T, theta):
        ...
        self.data_subset = np.zeros(self.N)
        self.data_subset[npr.randint(0, self.N, self.N / 10)] = 1
        for i in range(T - 1):
    ...
    def get_data_subset(self, i):
        theta = self.S[i]
        subset_sample_indx = npr.randint(0, self.N, size=self.num_sampled_z)
        L = np.exp(self.log_lkhd.comp_func(self.dataset[subset_sample_indx], theta))
        logB = self.log_bounding_function(theta, self.dataset[subset_sample_indx])
        B = np.exp(logB)
        self.data_subset[subset_sample_indx] = npr.binomial(1, 1 - B / L)
    ...
    def get_lkhd(self, theta, data):
        ...
        avgLogBound = (
            np.mean(self.log_lkhdMAP)
            + np.dot(self.meanGradMAP, h)
            + 0.5 * np.dot(np.dot(self.meanHessMAP.T, h), h)
            - R
        )
...
theta = np.array([1, 2])
norm_lkhd = Norm_lkhd()
fireflyMC = FlyMH(x, norm_lkhd, 0.1)
print(fireflyMC.data_subset)
fireflyMC.get_data_subset(0)
print(fireflyMC.data_subset)
